...
def ensure_packages_are_importable():
    """
    (FIXED V2) Creates empty __init__.py files in component directories
    and their subdirectories to make them fully importable.
    """
...
def load_translations(lang_code: str) -> dict:
    """
    Loads all locale .json files for a specific language into a single dictionary.
    """
    translations = {}
...
def _generate_markdown_table(headers, rows):
    """Helper function to generate a clean markdown table."""
...
def generate_component_doc(component_dir: str, component_id: str, manifest: dict, lang: str, translations: dict) -> dict:
    """
    (UPGRADED V3) Generates a complete and clean markdown file for one component.
    """
    ...
    output_path = os.path.join(DOCS_DIR, lang_path_segment, component_dir, doc_filename)
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(md_content)
    nav_path = os.path.join(component_dir, doc_filename).replace('\\', '/')
...
def main():
    ...
    config_lines = [
        ...
        "repo_url: https://github.com/awenkolayaudico/flowork",
        "repo_name: awenkolayaudico/flowork",
        "",
        "theme:",
        f"  name: material",
    ...
    for lang in LANGUAGES:
        ...
        link = "./" if lang == DEFAULT_LANG else f"./{lang}/"
